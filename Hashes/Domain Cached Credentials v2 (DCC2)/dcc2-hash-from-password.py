#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# File name          : dcc2-hash-from-password.py
...
if __name__ == '__main__':
    ...
    if password is not None:
        ...
        dcc1_hash = ctx.digest()
        print("   [+] Raw DCC1 hash: %s" % ctx.digest())
        print("   [+] Hex DCC1 hash: %s" % ctx.hexdigest())
        print()

        print("[+] Step 4: Applying PBKDF2-HMAC-SHA1 with %d rounds to DCC1 hash" % options.rounds)

        dcc2_hash = hashlib.pbkdf2_hmac("sha1", dcc1_hash, username, options.rounds)
        print("   [+] Raw DCC2 hash: %s" % dcc2_hash)
        hex_dcc2_hash = binascii.hexlify(dcc2_hash).decode('utf-8')[:32]
        print("   [+] Raw DCC2 hash: %s" % hex_dcc2_hash)
        print("   [+] Hashcat format: $DCC$%d#%s#%s" % (options.rounds, options.username, hex_dcc2_hash))
    else:
        pass
