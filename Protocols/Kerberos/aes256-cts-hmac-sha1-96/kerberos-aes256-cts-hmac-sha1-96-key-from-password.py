#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# File name          : kerberos-aes256-cts-hmac-sha1-96-key-from-password.py
...
if __name__ == '__main__':
    ...
    aes_ctx_ct2 = AES.new(key=K, mode=AES.MODE_CBC, iv=IV)
    CT2 = aes_ctx_ct2.encrypt(pad(CT1, AES.block_size))
    print("  | AES(key='%s').encrypt(CT1) = %s" % (K_hex, CT2))
    ...
    print("  | CT2[:16] = %s" % binascii.hexlify(CT2).decode('utf-8'))
    identity = options.username
    if len(options.domain) != 0:
        identity = options.domain.upper() + '\\' + identity
    print("  | Kerberos aes256-cts-hmac-sha1-96 key for '%s': %s" % (identity, binascii.hexlify(CT1+CT2).decode('utf-8')))
    print()
